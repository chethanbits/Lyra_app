...
Run as: uvicorn app_v2:app --reload --port 8000

GUEST: GET /day, /range, /heatmap (unchanged behavior).
PROFILE: GET /p/day, /p/range (birth_date, birth_time, pob_lat, pob_lon, pob_tz).
"""
...
def _settings(region: str, anchor: str, ayanamsa: str) -> guest.EngineSettings:
    try:
        ...
        raise HTTPException(status_code=400, detail="region must be NORTH_INDIA or SOUTH_INDIA")
    try:
        a = guest.AnchorMode(anchor.upper())
    except Exception:
        raise HTTPException(status_code=400, detail="anchor must be SUNRISE or NOW")
...
def _engine_error(e: Exception) -> HTTPException:
    if isinstance(e, guest.EngineError):
        return HTTPException(
            status_code=400,
            detail={"code": e.code.value, "message": e.message, "details": e.details},
        )
